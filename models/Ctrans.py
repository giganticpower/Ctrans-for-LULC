...
import math
from visual_transformer_noxin import FilterBasedTokenizer, Transformer, Projector
from eca_module import eca_layer

class _Ctrans(nn.Module):
    ...
    def __init__(self, inplanes, planes, lr_mult, weight_init_scale):
        ...
        self.eca_layer = eca_layer(512*3)
        self.token = FilterBasedTokenizer(512)
        self.transformer = Transformer(512)
        self.pro = Projector(512)
    ...
    def forward(self, x, y=None, z=None):
        ...
        if z is not None:
            ...
            # eca
            H_cat = self.eca_layer(H_cat)

            # Token-trans
            H_cat = self.token(H_cat)
            H_cat = self.transformer(H_cat)
            H_cat = self.pro(H_cat)
